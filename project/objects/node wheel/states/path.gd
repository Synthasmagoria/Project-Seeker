...
func physics_process(delta : float) -> String:
	...
	if is_instance_valid(_path_follow):
		...
		var _passed_pfs = _path_follow.test_move(_new_av * delta)
		if _passed_pfs.size() > 0:
			var _direction = sign(_new_av)
			for n in _passed_pfs:
				var _pcp = n as PathCouplingPoint2D
				if _pcp && is_couple_direction_adjacent(_path_follow, _direction, _pcp.coupling_point, _direction):
					# Calculate total travel distance
					var _remainder = abs(_new_av * delta)
