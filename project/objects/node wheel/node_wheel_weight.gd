...
func _process(delta: float) -> void:
	...
	var _next_pf_offset = _path_follow.offset + _new_av * get_physics_process_delta_time()
	...
	if _next_pf_offset <= 0.0:
		match path_reached_beginning_response:
			PathEndResponse.STOP:
				_new_av = (_next_pf_offset_clamped - _path_follow.offset) / get_physics_process_delta_time()
